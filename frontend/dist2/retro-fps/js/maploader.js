...
function MapLoader() {
	// TILETYPES is defined globally in data.js; registerWeapons() in data.js adds pickable* entries.
	const
		PI=QMATH.PI,
		PI2=PI*2,
		PI_2=PI/2;

	function getRaycasterValue(mod,key) { return mod[key] ?? RAYCASTER_DEFAULTS[key]; }
	...
	function getLogicTile(ret,x,y) {
		return ret.logicMap[(y*ret.mapWidth)+x] || { blockable: true };
	}
