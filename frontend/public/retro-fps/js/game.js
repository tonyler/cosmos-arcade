function Game() {
	...
	// Timers and triggers
	var gameTimer,readyGoTimer,triggers,teleports,gameState,abortTimer,autoEndTimer;
	// Game speed
		...
		shootables.forEach(player=>{
			if (player.playing&&!player.isDead) {
				...
				if (distance<maxdistance) {
					...
					// Attacker-auth: only fire giveDamage when the shooter is local
					if (fromplayer&&fromplayer.isLocal)
	...
	function checkExplodables(sprite,dx,dy,except,skipShootables) {
		// Shootables — skipped for remote bullets (attacker-auth: only attacker detects player hits)
		if (!skipShootables)
	...
	this.show=function(gamesettings) {
		...
		// Game speed unaware
		ABORT_TIME=FPS*3;
		AUTO_GAMEOVER_TIME=FPS*5; // auto-advance after 5s if player doesn't hold fire
		...
		maxMovementSpeed=2.8125/speedRatio; // ponytail: 50% slower than default 5
					...
					case TRIGGERS.bullet:{
						...
						if (!trigger.exploded) {
							...
							if (!trigger.exploded) {
								...
								// Attacker-auth: remote bullets skip player collision on victim's client
								var isRemoteBullet=netEnabled&&trigger.fromplayer&&!trigger.fromplayer.isLocal;
								trigger.exploded=checkExplodables(trigger.sprite,0,0,trigger.safeTimer?trigger.fromplayer:0,isRemoteBullet);
							}
						}
						...
						if (trigger.exploded||(trigger.timer<=0)) {
							...
							RC.removeSprite(trigger.sprite);
							// Attacker-auth: only attacker broadcasts EXPLODEBULLET (victim waits for this)
							if (netEnabled&&!trigger.nonet&&(!trigger.fromplayer||trigger.fromplayer.isLocal)) NETPLAY.broadcastEvent([NETEVENT_EXPLODEBULLET,trigger.bulletUid]);
						...
						if (gameModeData.gameRunning) {
							...
							if (control) {
								...
								if (weapon&&player.ammo&&!player.weaponReloading)
									if ((weapon.canHoldTrigger&&(control[player.fireButton]>0))||(control[player.fireButton]==1)) {
										...
										if (weapon.isRay) {
											...
											// ponytail: pitch blended into speedZ for local trajectory; remote sees flat (netplay sends by ID, not z-velocity)
											var _bt=BULLETS[weapon.bullet];
											if (sprite.pitch) _bt=Object.assign({},_bt,{speedZ:(_bt.speedZ||0)+sprite.pitch*-3});
											addBullet(sprite.x,sprite.y,sprite.angle,sprite.width*2,_bt,player,weapon.fragSymbol);
										}
								...
								if (control.aimPointer) {
									...
									// Vertical look (COD-style pitch)
									sprite.pitch=Math.max(-0.45,Math.min(0.45,sprite.pitch-control.aimPointer[1]*mouseSensitivity));
								}
								...
								// Elevated floor: update defaultZ based on tile type
								player.defaultZ = player.tile.elevationZ !== undefined ? player.tile.elevationZ : 0;

								// Jump (space = keyCode 32)
								if (CONTROLS.isKeyDown(32)&&!player.jumpVelocity&&sprite.z>=player.defaultZ)
									player.jumpVelocity=-0.048;
								if (player.jumpVelocity) {
									sprite.z+=player.jumpVelocity;
									player.jumpVelocity+=0.005;
									if (sprite.z>=player.defaultZ) { sprite.z=player.defaultZ; player.jumpVelocity=0; }
								}
							}
							...
							if (player.isLocal) {
								...
								if (gameTimer%STEPPING_TIME==0) {
									// Only apply tile damage when player is on the ground (not jumping over it)
									if (player.tile.onwalk && sprite.z >= player.defaultZ - 0.05) {
				...
				if (player.playing) {
					...
					if (weapon)	{
						...
						if (reticleColor&&(weapon.reticle||gameModeData.reticleMode)&&!player.weaponReloading) {
							var rx=QMATH.floor(player.reticleX+wavex), ry=QMATH.floor(player.reticleY+wavey);
							CANVAS.fillRect(ctx,reticleColor,1,rx-4,ry,4,1); // left
							CANVAS.fillRect(ctx,reticleColor,1,rx+3,ry,4,1); // right
							CANVAS.fillRect(ctx,reticleColor,1,rx,ry-4,1,4); // top
							CANVAS.fillRect(ctx,reticleColor,1,rx,ry+3,1,4); // bottom
						}
					}
			...
			case 100:{ // Game over
				if (TRANSITION.isFree) {
					if (readyGoTimer==0) {
						...
						autoEndTimer++;
						if (abortTimer>=ABORT_TIME||autoEndTimer>=AUTO_GAMEOVER_TIME) {
