...
Browser-based PvP arcade games with ATOM bets on Cosmos Hub. No installs, no downloads — just open the browser, connect Keplr, and play.
...
Cosmos Arcade is a lightweight gaming platform where you pick a game, put up some ATOM, and play a friend (or a stranger) for the pot. Funds are locked in a CosmWasm escrow contract on-chain. The winner is determined in the game — the contract pays out immediately after.
...
> INSERT IMAGE (pacman screenshot)
...
> INSERT IMAGE (void arena screenshot)
...
Everything runs in the browser. The server handles matchmaking and relays game state between players in real time. No game logic runs server-side — the client is authoritative.
...
| Games | Plain canvas / Three.js (no game engine) |
...
Drop a new entry in `frontend/src/lib/games.ts` — slug, title, description, thumbnail, and a lazy-loaded component. Nothing else needs to change. The matchmaking, escrow, and settlement system picks it up automatically.
